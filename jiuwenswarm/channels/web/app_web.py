...
class _SpaStaticHandler(SimpleHTTPRequestHandler):
    ...
    def _is_jiume_asset_route(self) -> bool:
        return urlparse(self.path).path.startswith("/_jiume/assets/")
    ...
    def _handle_jiume_asset_get(self, parsed) -> None:
        parts = [unquote(part) for part in parsed.path.split("/") if part]
        if len(parts) < 4 or parts[0] != "_jiume" or parts[1] != "assets":
            self._write_json(404, {"error": "not_found"})
            return
        twin_id = parts[2]
        ...
        try:
            ...
            target, mime_type = resolve_asset_path(twin_id, asset_name)
        except FileNotFoundError:
            self._write_json(404, {"error": "asset_not_found"})
            return
        except Exception as exc:  # noqa: BLE001
            self._write_json(403, {"error": "forbidden_asset", "detail": str(exc)})
            return
    ...
    def do_GET(self) -> None:  # noqa: N802
        ...
        if self._is_jiume_asset_route():
            self._handle_jiume_asset_get(parsed)
            return
