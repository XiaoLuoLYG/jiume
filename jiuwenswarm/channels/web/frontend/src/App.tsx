...
function initialNavFromLocation(): MainNavKey {
  // JiuMe opens /?panel=config so first-run runtime setup lands on Configuration.
  const params = new URLSearchParams(window.location.search);
  const panel = params.get('panel') || params.get('nav');
  if (panel === 'config' || panel === 'configpanel') {
    return 'configpanel';
  }
  return 'chat';
}

function initialConfigGroupFromLocation(): string | null {
  const params = new URLSearchParams(window.location.search);
  if (params.get('panel') === 'config') {
    return params.get('group') || 'model_default';
  }
  return null;
}
...
function AppContent() {
  ...
  const [activeNav, setActiveNav] = useState<MainNavKey>(() => initialNavFromLocation());
  ...
  const [configInitialExpandGroup, setConfigInitialExpandGroup] = useState<string | null>(() => initialConfigGroupFromLocation());
  useEffect(() => {
    const params = new URLSearchParams(window.location.search);
    if (params.get('panel') === 'config') {
      setActiveNav('configpanel');
      setConfigInitialExpandGroup('model_default');
    }
  }, []);
  useEffect(() => {
